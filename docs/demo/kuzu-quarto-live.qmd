...
G = toNetworkx(response,directed = True)
g = Network(height="480px",notebook=True, cdn_resources='remote',directed = True,neighborhood_highlight = True)
g.from_nx(G)
...
for i in range(len(g.nodes)): 
  if g.nodes[i]["_LABEL"] == "Author":
    g.nodes[i]["label"] = g.nodes[i]["Author"]
  elif g.nodes[i]["_LABEL"] == "Document":
    g.nodes[i]["label"] = g.nodes[i]["FileName"]
    # Add hyperlink
    g.nodes[i]["title"] = f"""<a href="{g.nodes[i]["URL"]}" target="_blank">{g.nodes[i]["URL"]}</a>"""
